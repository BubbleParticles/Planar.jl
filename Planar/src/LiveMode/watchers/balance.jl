    ...
    pipeline = Rocket.interval(10, 10) |>
        Rocket.map(Nothing, _ -> begin
            ...
            if TimeTicks.now() - last > Second(60)
                @warn "balance watcher: forcing fetch due to stall" last TimeTicks.now() s
...
function _balance_watch_setup!(state)
    ...
    sub = Rocket.subscribe!(state.buf_subject, Rocket.lambda(
        on_next = v -> begin
            if v isa Exception
        ...
        on_error = e -> @warn("balance watcher: buf subscription error", exception = (e, catch_backtrace()))
    ))
    push!(state.subs, sub)
    nothing
end
...
function _w_balance_func(s, w, attrs)
    ...
    proc_sub = Rocket.subscribe!(state.process_subject, Rocket.lambda(
        on_next = w -> process!(w),
        on_error = e -> @warn("balance watcher: process subscription error", exception = (e, catch_backtrace()))
    ))
...
function _balance_task!(w)
    f = _tfunc(w)
    w[:balance_task] = @start_task IdDict() begin
        try
            while isstarted(w)
                try
                    f(w)
                    ...
                    if e isa InterruptException
                        ...
                        @error "watch balance: fetch failed" exception = (e, catch_backtrace()) _module = LogWatchBalance
                    end
                end
            end
        finally
...
function Watchers._stop!(w::Watcher, ::CcxtBalanceVal)
    ...
    if istaskrunning(bt)
        ...
        while !istaskdone(bt) && TimeTicks.now() < to_time
            sleep(0.1)
        end
        if !istaskdone(bt)
            @warn "balance watcher: stopping task by exception" w = w.name
            kill_task(bt)
        end
    end
    _unsubscribe_subs!(w)
...
function Watchers._process!(w::Watcher, ::CcxtBalanceVal; fetched=false)
    ...
    date = @something pytodate(data, eid) TimeTicks.now()
...
function watch_balance!(s::LiveStrategy; interval=st.throttle(s), wait=false)
    ...
    while wait && just_started && _lastprocessed(w) == DateTime(0)
        ...
        Rocket.subscribe!(w.beacon.process |> Rocket.take(1), Rocket.lambda(
            on_next = _ -> nothing,
            on_error = e -> @warn("balance watcher: initial wait error", exception = (e, catch_backtrace()))
        ))
    end
