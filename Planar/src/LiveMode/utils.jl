...
macro timeout_start(start=nothing)
    esc(:(timeout_date = @something $start TimeTicks.now() + waitfor))
end
...
"""
macro timeout_now()
    esc(:(max(Millisecond(0), timeout_date - TimeTicks.now())))
end
        ...
        if istaskrunning(task)
            ...
            t = @start_task IdDict() begin
                try
                    ...
                    @error "reset asset tasks: failed to kill task" name = name exception = (e, catch_backtrace())
                end
            end
    ...
    for (order, task) in tasks.byorder
        if istaskrunning(task)
            t = @start_task IdDict() try
                ...
                e isa InterruptException && rethrow(e)
                @error "reset asset tasks: failed to kill order task" order = this_order exception = (e, catch_backtrace())
            end
            push!(completion_tasks, t)
        end
    end
    ...
    if reset
        for (name, task) in tasks.tasks
            if istaskrunning(task)
                ...
                waitforcond(() -> !istaskrunning(task), Minute(1))
                if istaskrunning(task)
                    @warn "strategy task did not terminate, killing" _module = LogTasks account name
                    kill_task(task)
                end
            end
        end
...
function stop_all_strategy_tasks(s::RTStrategy; reset=false, kwargs...)
    ...
    if reset
        @sync for acc in keys(accounts)
            @async try
                stop_strategy_tasks(s, acc; reset, kwargs...)
            catch e
                e isa InterruptException && rethrow(e)
                @error "stop all strategy tasks: account failed" account = acc exception = (e, catch_backtrace())
                rethrow(e)
            end
        end
    else
...
function stop_all_tasks(s::RTStrategy; reset=true)
    ...
    for (k, v) in pairs(s.attrs)
        if v isa Watcher && isstarted(v)
            ...
            try
                ...
                e isa InterruptException && rethrow(e)
                @error "strategy: error stopping user watcher" _module = LogTasks k exception = (e, catch_backtrace())
            end
        end
    end
    ...
    try
        stop_all_asset_tasks(s; reset)
    catch e
        e isa InterruptException && rethrow(e)
        @error "stop asset tasks error:" _module = LogWait exception = (e, catch_backtrace())
        rethrow(e)
    end
    try
        stop_all_strategy_tasks(s; reset)
    catch e
        e isa InterruptException && rethrow(e)
        @error "stop strategy tasks error:" _module = LogWait exception = (e, catch_backtrace())
        rethrow(e)
    end
    @debug "strategy: stopping handlers" _module = LogTasks s = nameof(s)
    stop_handlers!(s)
    @debug "strategy: stopping paper order tasks" _module = LogTasks s = nameof(s)
    try
...
function live_sync_start!(s::LiveStrategy; first_start)
    ...
    @sync begin
        @async try
            waitwatcherprocess(balance_watcher(s))
        catch e
            e isa InterruptException && rethrow(e)
            @error "live strat: balance watcher wait failed" exception = (e, catch_backtrace())
        end
        if s isa MarginStrategy
            @async try
                waitwatcherprocess(positions_watcher(s))
            catch e
                e isa InterruptException && rethrow(e)
                @error "live strat: positions watcher wait failed" exception = (e, catch_backtrace())
            end
        end
    end
...
function stop!(s::LiveStrategy; kwargs...)
    ...
    try
        ...
        e isa InterruptException && rethrow(e)
        @error "stop! failed" exception = (e, catch_backtrace()) _module = LogTasks
    end
