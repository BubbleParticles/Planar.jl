        ...
        ii, orders_f, ((ids; symbol) -> begin
            @sync for id in ids
                @async try
                    ...
                    e isa InterruptException && rethrow(e)
                    @error "cancel all orders: single failed" id exception = (e, catch_backtrace())
                end
            end
        end)
    )
        ...
        function ccxt_orders_single(ii; ids, side=BuyOrSell, kwargs...)
            ...
            @sync for id in ids
                @async try
                    ...
                    e isa InterruptException && rethrow(e)
                    @error "orders single: fetch failed" id exception = (e, catch_backtrace())
                end
            end
            out
        end
...
cancel_loop_func(exc, cancel_single_f) = begin
    cancel_loop(ids; symbol) = @sync for id in ids
        @async try
            _execfunc(cancel_single_f, id; symbol)
        catch e
            e isa InterruptException && rethrow(e)
            @error "cancel loop: single cancel failed" id exception = (e, catch_backtrace())
        end
    end
end
...
find_trades_or_since(a, ii, id::String)::Tuple{DateTime,Option{Any}} = begin
    ...
    try
        ...
        e isa InterruptException && rethrow(e)
        @error "find_trades_or_since failed" exception = (e, catch_backtrace()) _module = LogCcxtFuncs
        return default_since(), nothing
    end
end
