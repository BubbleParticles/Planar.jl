...
function start_strategy(cl::TelegramClient, s; text, chat_id, kwargs...)
    ...
    if isrunning(s)
        ...
        try
            ...
            @error "start_strategy: notification failed for $(name)" exception = (e, catch_backtrace())
        end
    end
    true
end
    ...
    if !isrunning(s)
        ...
        function dostop()
            try
                ...
                @error "dostop failed for $(nameof(s))" exception = (e, catch_backtrace())
            end
        end
        errormonitor(@async (sleep(delay); dostop()))
    end
end
...
function status(cl::TelegramClient, s; text, chat_id, kwargs...)
    thr = throttle(s)
    snap = lm.snapshot(s.universe)
    n_assets = length(snap)
    first_a = first(snap).asset.raw
    last_a = last(snap).asset.raw
...
function _rolling(cl::TelegramClient, s; period, text, chat_id)
    ...
    try
        cur = nameof(s.cash)
        asset_data = []
        for ii in lm.snapshot(s.universe)
...
function balance(cl::TelegramClient, s; text, chat_id, kwargs...)
    prices = zeros(Float64, length(lm.snapshot(s.universe)))
    @sync for (i, ii) in enumerate(lm.snapshot(s.universe))
        @async prices[i] = try
            st.lastprice(ii)
        catch e
    ...
    prices_dict = Dict(ii => prices[i] for (i, ii) in enumerate(lm.snapshot(s.universe)))
