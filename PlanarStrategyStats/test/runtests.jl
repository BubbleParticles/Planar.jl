module PlanarStrategyStatsTests

using Test
using PlanarCore
using PlanarStrategyStats
using PlanarStrategyStats: diffn, momentum, slopetoangle, mlr_slope, slopeangle, fltsummary, filterminmax, is_slopebetween
using PlanarStrategyStats: up_successrate, down_successrate, is_peaked, is_bottomed, is_uptrend
...
@testset "PlanarStrategyStats" begin
    ...
    @testset "diffn matrix" begin
        ...
        dX = PlanarStrategyStats.diffn(X; n=1)
        @test size(dX) == size(X)
        @test isnan(dX[1, 1])
        @test dX[2, 1] ≈ 1.0
        @test dX[3, 1] ≈ 1.0
    ...
    @testset "queries.jl functions" begin
        ...
        @testset "average_roc" begin
            result = PlanarStrategyStats.Query.average_roc(mrkts)
        ...
        @testset "cbot / cpek" begin
            ...
            result_cbot = PlanarStrategyStats.Query.cbot(hs, mrkts; n=30:-3:15, min_n=1)
            @test result_cbot isa DataFrame

            result_cpek = PlanarStrategyStats.Query.cpek(hs, mrkts; n=30:-3:15, min_n=1)
    ...
    @testset "slopefilter parameter name" begin
        ...
        @test hasmethod(PlanarStrategyStats.slopefilter, Tuple{AbstractDict})
        # Can't call slopefilter without exchange, but verify the dict dispatch exists
    end
    @testset "maxmin edge cases" begin
        # Normal-sized frame should not error and should populate columns
        df = test_ohlcv(120)
        res = PlanarStrategyStats.maxmin(df; window=100)
        @test res isa DataFrame
        @test :maxima in propertynames(res) && :minima in propertynames(res)
        # Short frame (fewer rows than window+2) must not throw a BoundsError
        short = test_ohlcv(5)
        @test PlanarStrategyStats.maxmin(short; window=100) isa DataFrame
        # Frame whose tail window exceeds bounds must also not throw
        mid = test_ohlcv(150)
        @test PlanarStrategyStats.maxmin(mid; window=100) isa DataFrame
    end
