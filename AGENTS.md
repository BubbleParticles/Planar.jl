...
36. **`first(exc, :watchTickersForSymbols, :watchTickers)` always returns non-nothing from CcxtGateway**: The gateway's `has` endpoint returns `true` for ALL websocket methods, even unsupported ones. Using `!isnothing(first(exc, :watchTickers))` as a websocket-detection heuristic always evaluates to `true`, forcing the watcher into the websocket path which sets `_tfunc = () -> check_task!(w)` (a no-op). Fix: check an explicit user override `attrs[:iswatch]` instead, defaulting to `false`.
