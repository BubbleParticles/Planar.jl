...
38. **Verify ccxt method signatures before passing kwargs — leverage and margin mode are separate ccxt methods**: `set_margin_mode(self, marginMode, symbol=None, params={})` does NOT accept a leverage parameter (verified: `ccxt/binance.py:11648`, `ccxt/base/exchange.py:3684`). Leverage is set via `set_leverage(self, leverage, symbol=None, params={})` (`binance.py:11601`). When a Julia wrapper accepts `kwargs...` but the downstream dispatch constructs the request body explicitly (dropping kwargs), any kwarg is silently absorbed — treat `kwargs...` as a code smell and verify the body dict construction. Also, ccxt's Binance `set_margin_mode` raises `ArgumentsRequired` if `symbol` is None, so Binance-specific overrides must keep `symbol=""` default to align with…[truncated]
...
75. **`marginmode!` kwargs must NOT be forwarded to `dosetmargin`/`call_exchange` — verify the body dict is constructed explicitly**: The `marginmode!` dispatch in `PlanarCore/src/Exchanges/leverage.jl` accepts `kwargs...` but `dosetmargin(exc, mode_str, symbol; kwargs...)` constructs the request body as `Dict("marginMode" => mode_str, "symbol" => symbol)` — `kwargs...` is never forwarded to `call_exchange`. Any kwarg passed to `marginmode!` (e.g. `lev=leverage(pos)`) is silently absorbed and never reaches the exchange. This is correct behavior: ccxt's `set_margin_mode(self, marginMode, symbol=None, params={})` does NOT accept a leverage parameter (verified in ccxt source: `Planar/.venv/lib/python3.14/site-packages/ccxt/binance.py:11648` and `ccxt/base/exchange.py:3684`). Leverage is a separate ccxt method — `set_leverage(self, leverage, symbol=None, params={})` (`binance.py:11601`). In the sync flow, `leverage!(pos, lev)` sets the local position leverage and `leverage!` is called separately for the exchange via `call!`/`UpdateLeverage`. **When adding a kwarg to a `marginmode!`-style dispatch, verify the callee's body construction does not silently drop it — if it does, the kwarg is dead weight and should be removed.**

76. **Binance `set_margin_mode` requires a non-None `symbol`**: ccxt's Binance `set_margin_mode` raises `ArgumentsRequired` if `symbol` is None (`binance.py:11660-11661`). The generic `marginmode!` dispatch passes `symbol=""` by default, but a Binance-specific override with `symbol` as a required positional argument would `MethodError` when called as `marginmode!(binance, "isolated")` (no symbol). Align the override's method signature with the generic default (`symbol=""`) so dispatch falls through correctly. The override must still accept and forward the `hedged` keyword for API compatibility.
---
