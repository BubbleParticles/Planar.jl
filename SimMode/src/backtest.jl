...
function start!(
    ...
    if use_gpu && oneAPI.functional()
        update_func = update_gpu!
        call_func = call_gpu!
        desc = "Backtesting (GPU)"
    else
        update_func = update!
        call_func = call!
        desc = "Backtesting"
    ...
    with_logger(logger) do
        if show_progress
            ...
            @withpbar! trimmed_range desc=desc begin
                for date in ctx.range
                    isoutof_orders(s) && begin
                        @deassert all(iszero(ai) for ai in universe(s))
                        break
                    end
                    update_func(s, date, update_mode)
                    call_func(s, date, ctx)
                    # Update stats
...
_todate(s) = begin
    ...
    for ai in s.universe
        ...
        if this_date isa oneAPI.oneArray
            this_date = oneAPI.Array(this_date)[1]
        end
        if this_date > to
            to = this_date
        end
...
"""
function start!(s::Strategy{Sim}, from::DateTime, to::DateTime=_todate(s); use_gpu=false, kwargs...)
    ctx = Context(Sim(), s.timeframe, from, to)
    start!(s, ctx; use_gpu=use_gpu, kwargs...)
end
