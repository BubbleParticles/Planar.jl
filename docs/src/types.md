...
The [Julia](https://julialang.org/) main `Dates` package is never imported directly. It is instead exported by the package `TimeTicks`, which, among many utility functions, overrides the `now` function to always use the `UTC` timezone.
