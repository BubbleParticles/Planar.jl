...
using PlanarCore.Misc: config, DFT
...
"""
function _local_extrema(x::AbstractVector, order::Int, ismax::Bool, threshold::Real)
    n = length(x)
    out = falses(n)
    order <= 0 && return out
    @inbounds for i in (order + 1):(n - order)
        xi = x[i]
        # Compare against the `order` neighbours on each side.
        ok = true
        prom = zero(xi)
        for j in 1:order
            lo = x[i - j]
            hi = x[i + j]
            if ismax
                xi <= lo && (ok = false; break)
                xi <= hi && (ok = false; break)
                prom = max(prom, xi - lo, xi - hi)
            else
                xi >= lo && (ok = false; break)
                xi >= hi && (ok = false; break)
                prom = max(prom, lo - xi, hi - xi)
            end
        end
        ok && prom > threshold && (out[i] = true)
    end
    out
end

function maxima(x::AbstractVector; order::Int=1, threshold::Real=0.0)
    _local_extrema(x, order, true, threshold)
end

function minima(x::AbstractVector; order::Int=1, threshold::Real=0.0)
    _local_extrema(x, order, false, threshold)
end

function maxmin(df; order=1, threshold=0.0, window=100)
    df[!, :maxima] .= NaN
    df[!, :minima] .= NaN
    nrow(df) < window + 2 && return df
    dfv = @view df[(window + 2):end, :]
    price = df.close
    @eachrow! dfv begin
        stop = row + window
        stop > nrow(df) && continue
        df.timestamp[stop] < :timestamp || continue
