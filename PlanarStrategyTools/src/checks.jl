...
"""
belowtotal(s, ii, p; qt=s.qt_base) = collateral(p) < max(ii.limits.amount.min, current_total(s) * qt)

@doc """ Verify if free cash is above the entry cost minimum

$(TYPEDSIGNATURES)
"""
hasentrycash(s, ii) = freecash(s) > ii.limits.cost.min

@doc """ Assess if amount exceeds the minimum exit amount

$(TYPEDSIGNATURES)
"""
hasexitcash(amt, ii) = amt > ii.limits.amount.min
