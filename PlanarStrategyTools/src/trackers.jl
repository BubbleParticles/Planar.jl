...
"""
function trackpnl!(s, ii, ats, ts; interval=10s.timeframe, pnl_func=inst.pnlpct)
    pside = posside(ii)
    pnl = s[:pnl][ii]
    pnl_ts = pnl[1][]
    if pnl_ts < ats
        if !isnothing(pside)
            @deassert isopen(ii)
            close = closeat(ii, ats)
            pnl[1][] = ats
            oti.fit!(pnl[2], pnl_func(ii, pside, close))
...
function initpnl!(s, uni=s.universe; n=100, ma=oti.SMA{DFT})
    s[:pnl] = LittleDict(ii => (Ref(DateTime(0)), ma(period=n)) for ii in uni)
end
...
"""
function copypnl!(s, ii, s_sim, ai_sim)
...
function initlev!(s)
    ...
    s[:lev] = LittleDict(
        ii => (time=DateTime(0), raw_val=lev, value=lev) for ii in s.universe
    )
end

levtuple(s, ii) = s[:lev][ii]
getlev(s, ii) = levtuple(s, ii).value
function iszerolev(s, ii, ts; timeout=Day(1))
    tup = levtuple(s, ii)
...
"""
function tracklev!(s, ii, ats; dampener=default_dampener)
    pnl_ats, pnl = getpnl(s, ii)
    this_lev = levtuple(s, ii)
    if this_lev.time < ats
        μ = @coalesce pnl.value 0.0
        vals = pnl.input_values.value
        n = length(vals)
        if n < 2
            # Not enough samples for a variance estimate — fall back to default leverage.
            def = s[:def_lev]
            raw_val, value = def, def
        else
            s2 = ((vals .- μ) .^ 2 |> sum) / (n - 1)
            k = μ / s2
            raw_val, value = if isnan(k) || isinf(k)
                def = s[:def_lev]
                def, def
            elseif k <= 0.0
                0.0, 0.0
        ...
        s[:lev][ii] = (; time=pnl_ats[], raw_val, value)
    end
end

## QT

function _normat(s, ii, ats; mn, mx, f=volumeat)
    (f(s, ii, ats) - mn) / (mx - mn)
end

function _volumeat(_, ii, ats)
    data = ohlcv(ii)
...
function initqt!(s)
    ...
    let v = inv(length(marketsid(s)))
        attrs[:qt] = LittleDict(ii => v for ii in s.universe)
    end
...
"""
function trackqt!(s, ii, ats; f=_qtvolumeat)
    local mn, mx
    ex = s[:qt_ext]
    if ex[1] == ats
        mn, mx = ex[2], ex[3]
        ...
        ex[1] = ats
        mn, mx = extrema(f(s, ii, ats) for ii in s.universe)
        ex[2] = mn
        ex[3] = mx
    end
    v = _normat(s, ii, ats; mn, mx, f)
    s[:qt][ii] = if isfinite(v)
        v
    else
        max(0.0, s[:qt_base])
    end
...
"""
function track_expectancy!(s, ii)
    _, pnl = getpnl(s, ii)
    n_wins = 0
    tot_wins = 0.0
    n_losses = 0
...
function initcd!(s)
    ...
    s.attrs[:cd] = LittleDict(ii => DateTime(0) for ii in s.universe)
end
...
"""
function trackcd!(s, ii, ats, ts)
    _, pnl = getpnl(s, ii)
    mean_pnl = mean(pnl)
    s[:cd][ii] = ts + s.self.cdfrompnl(s, mean_pnl)
end

function isidle(s, ii, ats, ts)
    ts < s[:cd][ii]
end
...
function inittrends!(s, trends)
    attrs = s.attrs
    for k in trends
        @lget! attrs k LittleDict{InstrumentInstance,Trend}(
            ii => MissingTrend for ii in s.universe
        )
    end
end

isuptrend(s, ii, sig_name) = signal_trend(s, ii, sig_name) === Up
isdowntrend(s, ii, sig_name) = signal_trend(s, ii, sig_name) === Down
...
Checks if the asset `ii` is trending at time `ats` for the signal `sig_name` in the strategy `s`.
...
"""
function istrending!(s::Strategy, ii::InstrumentInstance, ats::DateTime, sig_name; func=cmptrend)
    ov = ohlcv(ii, signal_timeframe(s, sig_name))
...
#     s[:slope] = LittleDict(ii => (; time=Ref(DateTime(0)),
#         value=LinReg()) for ii in s.universe)
# end
# slope!(attrs, ii, ats) = begin
#     data = ohlcv(ii)
#     date, os = attrs[:slope][ii]
#     tf = attrs[:timeframe]
