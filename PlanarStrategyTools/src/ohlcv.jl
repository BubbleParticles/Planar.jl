...
function copyohlcv!(s_dst::T, s_src::T) where {T<:Strategy}
    ...
    dst_ai = Dict(raw(ii) => ii for ii in s_dst.universe)
    for ii in s_src.universe
        if raw(ii) ∈ syms_to
            for (tf, ov) in ii.data
                dst_ai[raw(ii)].data[tf] = ov
            end
        end
    end
end
...
"""
function copyohlcv!(ai_dst::T1, ai_src::T2) where {T1,T2<:InstrumentInstance}
...
"""
function isstaleohlcv(s::RTStrategy, ii; ats, tf, backoff)
    islast, last_date = islastts(s, ii, ats, tf)
    if islast
        if last_date > backoff[ii]
            backoff[ii] = last_date
        else
            @debug "surge: candle already processed" ii maxlog = 1 backoff[ii]
            return true
        end
    else
        @debug "surge: missing ohlcv" ii
        return true
    end
    return false
end
