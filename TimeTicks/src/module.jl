...
_parse_timeframe(s) = begin
    ...
    m === nothing && throw(ArgumentError("Invalid timeframe format: $s"))
    n_str = m[1]
    t = m[2]
    n = parse(Int, n_str)
    ...
    mul = 0
    if t == "ms"
        return TimeFrame(Millisecond(n))
    elseif t == "m"
        t = "T"
    ...
    elseif t == "y" || t == "Y"
        mul = 365
    # s, h, d, m (after conversion), min stay as their native units
    elseif t in ("s", "S", "h", "H", "d", "D", "T", "min")
        # keep as is
    else
        throw(ArgumentError("Unknown timeframe unit: $t in $s"))
    end
    if mul > 0
        t = "d"
        n = n * mul
...
```
"""
function compact(s::Period)::Period
