    ...
    if msg_type == "update"
        ...
        if callback !== nothing
            try
                ...
                @error "WebSocket callback error" subscription_id = sub_id exception = (e, catch_backtrace())
            end
        end
...
function connect!(client::GatewayWSClient)
    ...
    # Create a monitored task that logs any unhandled exceptions.
    # The task ref is stored in client.task so disconnect! can wait on it.
    client.task = Task() do
        try
            ...
            # Only capture connection establishment errors (before barrier notify)
            if conn_error[] === nothing
                conn_error[] = e
            end
            @error "WebSocket task failed" exception = (e, catch_backtrace())
            notify(barrier)
        finally
    ...
    # Wait for either the connection to be established or a connection error
    wait(barrier)
    if conn_error[] !== nothing
...
function disconnect!(client::GatewayWSClient)
    ...
    # Wait for the read loop task to finish its cleanup (finally block)
    task = client.task
    if task !== nothing && istaskstarted(task) && !istaskdone(task)
        wait(task)
    end
    client.task = nothing
end
