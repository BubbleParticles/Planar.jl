...
function trades_pnl(ai::AssetInstance; cum_bal=@cumbal(), returns=nothing, kwargs...)
...
function trades_stats(s::Strategy; extended=false, since=DateTime(0))
    ...
    for ai in s.universe
        ...
        if since >= first(ai.history).date
            hist = filter(x -> x.date >= since, copy(ai.history))
            isempty(hist) && continue
            ai_filtered = AssetInstance(ai.attrs, ai.asset, ai.data, hist, ai.lock, ai._internal_lock, ai.cash, ai.cash_committed, ai.exchange, ai.longpos, ai.shortpos, ai.lastpos, ai.limits, ai.precision, ai.fees)
