...
async def create_order(self, *args, **kwargs):
    ...
    try:
        ...
        # Honor requested amount and price when provided
        ...
        # Determine last market price to decide whether limit is immediately fillable
        last_price = None
        try:
            if symbol_str:
        ...
        if ot == 'market':
            ...
            if filled > 0:
                ts = _now_ms()
                price_used = ord.get('price', last_price) if last_price is not None else ord.get('price', 0.0)
