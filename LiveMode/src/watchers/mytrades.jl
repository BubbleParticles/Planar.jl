...
function define_trades_loop_funct(s::LiveStrategy, ai, exc; exc_kwargs=(;))
    ...
    if !isnothing(watch_func) && s[:is_watch_mytrades]
        ...
        last_date = isempty(ai.history) ? attr(s, :is_start, TimeTicks.now()) : last(ai.history).date
        since = Ref(last_date)
        startup = Ref(true)
        eid = exchangeid(ai)
...
function handle_trade_updates_errors!(e, ai, iswatch)
    if e isa InterruptException || (iswatch && e isa InvalidStateException)
        ...
        @error "watch trades: error (task termination?)" exception = (e, catch_backtrace()) _module = LogWatchTrade raw(ai)
    end
    if !iswatch
        sleep(1)
    end
