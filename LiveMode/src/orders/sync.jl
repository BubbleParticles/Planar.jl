...
function check_orders_sync(s::LiveStrategy)
    try
        ...
        @sync for ai in s.universe
            @async try
                for o in fetch_open_orders(s, ai)
                    push!(exc_ids, resp_order_id(o, eid, String))
                end
            catch e
                e isa InterruptException && rethrow(e)
                @error "check orders sync: fetch failed" ai = raw(ai) exception = (e, catch_backtrace())
            end
...
function live_sync_closed_orders!(s::LiveStrategy; kwargs...)
    @sync for ai in universe(s)
        @async try
            live_sync_closed_orders!(s, ai; kwargs...)
        catch e
            if e isa InterruptException
                rethrow(e)
            end
            @error "sync closed orders: error for $(raw(ai))" exception=(e, catch_backtrace())
        end
    end
end
...
function live_sync_open_orders!(s::LiveStrategy; kwargs...)
    @sync for ai in universe(s)
        @async try
            live_sync_open_orders!(s, ai; kwargs...)
        catch e
            if e isa InterruptException
                rethrow(e)
            end
            @error "sync open orders: error for $(raw(ai))" exception=(e, catch_backtrace())
        end
    end
end
