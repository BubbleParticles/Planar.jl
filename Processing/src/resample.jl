...
function resample(mkts::AbstractDict{String,PairData}, timeframe; progress=false, lk = ReentrantLock())
    ...
    try
        Threads.@threads for (name, pair_data) in collect(mkts)
            v = try
                PairData(name, timeframe, resample(pair_data, timeframe), nothing)
            catch e
...
function upsample(df::AbstractDataFrame, large_tf::TimeFrame, small_tf::TimeFrame)
    large_td = timefloat(large_tf.period)
    small_td = timefloat(small_tf.period)
    large_td > small_td || throw(ArgumentError("upsample: large_tf must be greater than small_tf"))
    large_td % small_td == 0 || throw(ArgumentError("upsample: large_tf ($(large_tf.period)) must be a multiple of small_tf ($(small_tf.period))"))
    n = Int(large_td ÷ small_td)
    n_large = nrow(df)
    n_small = n_large * n
