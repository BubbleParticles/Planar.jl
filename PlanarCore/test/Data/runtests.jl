...
using PlanarCore.Data: DictStore, PairData, _save_ohlcv, _load_ohlcv, _check_contiguity
...
@testset "load_ohlcv concurrency (no data race) and default_value" begin
    # Build an in-memory ZarrInstance
    store = DictStore()
    g = zgroup(store, "")
    zi = ZarrInstance("test_store", store, g)
    exc_name = "test"
    tf = "1m"
    ...
    # Concurrent load must return all pairs (no dropped/duplicated entries)
    for _ in 1:10
        out = load_ohlcv(zi, exc_name, pairs, tf)
        @test length(out) == length(pairs)
        @test Set(keys(out)) == Set(pairs)
    end

    # default_value on a function with no zero-arg method returns nothing (the
    # uninhabited Union{} return type must be skipped, not dereferenced).
    nozeroarg() = error("no zero-arg method")
    @test Data.default_value(nozeroarg) === nothing
end
