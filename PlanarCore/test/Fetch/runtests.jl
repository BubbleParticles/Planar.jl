...
@testset "Data conversion" begin
    ...
    @testset "_update_orderbook! logs failure instead of dying silently" begin
        # Build an exchange and seed markets so the orderbook path is reachable
        ob_exc = Exchange(:test_ob)
        ob_exc.markets["BTC/USDT"] = Dict{String,Any}(
            "id" => "BTC/USDT", "type" => "spot", "base" => "BTC", "quote" => "USDT",
            "taker" => 0.001, "maker" => 0.001,
            "precision" => Dict("amount" => 0.0001, "price" => 0.01),
            "limits" => Dict("amount" => Dict("min" => 0.0001, "max" => 1000.0),
                             "price" => Dict("min" => 0.01, "max" => 1000000.0),
                             "cost" => Dict("min" => 1.0, "max" => 10000000.0)))
        push!(ob_exc.types, :spot)
        ob = Fetch._orderbook(10)
        # Force the gateway fetch to fail (500) so call_exchange raises
        ExchangeTypes.CcxtGateway.Rest.set_http_post!((url; kwargs...) -> begin
            if occursin("fetchOrderBook", url)
                return HTTP.Response(500, JSON3.write(Dict("error" => "boom")))
            end
            return HTTP.Response(200, JSON3.write(Dict("result" => nothing)))
        end)
        # non-init path: runs async, polls briefly, must not crash and must reset busy
        ret = Fetch._update_orderbook!(ob_exc, ob, "BTC/USDT", Fetch.L1, 10; init=false)
        @test ret === ob
        sleep(0.6)
        @test ob.busy[] == false
        # order book arrays are pre-sized (undef); on failure they remain unpopulated.
        # The key regression: no exception escapes and busy is reset (the error is logged,
