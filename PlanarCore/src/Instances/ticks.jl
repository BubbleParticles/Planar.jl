...
"""
function ticks(ai::AssetInstance)
    get(ohlcv_dict(ai), TICK_TIMEFRAME, empty_ohlcv())
end
...
"""
function setticks!(ai::AssetInstance, df::DataFrame)
    ohlcv_dict(ai)[TICK_TIMEFRAME] = df
...
timeframe.
"""
_ohlcv_keys(ai::AssetInstance) =
    (k for k in keys(ohlcv_dict(ai)) if k != TICK_TIMEFRAME)
...
`ArgumentError`. The result is cached in `ai.attrs[:_tick_order]` keyed by the
...
"""
function _check_ticks_ordered!(ai::AssetInstance)
    ts = ticks(ai).timestamp
    isempty(ts) && return true
    fp = (first(ts), last(ts), length(ts))
    cached = get(ai.attrs, :_tick_order, nothing)
    ...
    for i in 2:length(ts)
        if ts[i] < ts[i - 1]
            throw(
                ArgumentError(
                    "ticks for $(raw(ai)) not properly ordered at index $i: $(ts[i - 1]) > $(ts[i])",
                ),
            )
        end
    end
    ai.attrs[:_tick_order] = (fp, true)
    return true
end
