...
When called without an explicit value, the entry price is inferred from the
position notional and cash (position size). `cash(po)` is a `Cash` wrapper, so
it must be unwrapped via `value` before division; a zero cash (closed or freshly
created position) would otherwise yield `Float64 / Cash` -> MethodError or a
divide-by-zero -> Inf. Guard it to 0.0 in that case.
"
function entryprice!(po::Position, v=let c = value(cash(po))
    iszero(c) ? 0.0 : abs(notional(po) / c)
end)
    po.entryprice[] = v
end
