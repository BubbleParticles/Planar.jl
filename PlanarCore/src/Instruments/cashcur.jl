...
struct Cash{S,T} <: AbstractCash
    value::Ref{T}
    lock::ReentrantLock
    Cash{C,N}(val) where {C,N} = new{C,N}(val, ReentrantLock())
    Cash(s, val::R) where {R} = new{Symbol(uppercase(string(s))),R}(Ref(val), ReentrantLock())
    function Cash(_::Cash{C,N}, val::R) where {C,N,R}
        new{C,N}(Ref(convert(N, val)), ReentrantLock())
    end
end
...
@doc """Macro to instantiate `Cash` statically with zero value.
...
*(a::Cash{S}, b::Cash{S}) where {S} = value(a) * value(b)
/(a::Cash{S}, b::Cash{S}) where {S} = value(a) / value(b)
# `+`/`-` of same-currency Cash must preserve the `Cash{S}` wrapper:
# callers (Instances `cash!`/`add!`/`sub!`, Collections aggregations) feed
# the result back into `Cash`-typed APIs, so a bare `Float64` would
# crash downstream with `MethodError: value(::Float64)`.
# (`*`/÷/`/` correctly return scalars — pure ratio/number ops.)
+(a::Cash{S}, b::Cash{S}) where {S} = Cash(S, value(a) + value(b))
-(a::Cash{S}, b::Cash{S}) where {S} = Cash(S, value(a) - value(b))
function Base.isapprox(a::C, b::C; kwargs...) where {C<:AbstractCash}
    isapprox(value(a), value(b); atol=ATOL, kwargs...)
end

add!(c::Cash, v, args...; kwargs...) = (lock(c.lock) do; _fvalue(c)[] += v; end; c)
sub!(c::Cash, v, args...; kwargs...) = (lock(c.lock) do; _fvalue(c)[] -= v; end; c)
function atleast!(c::AbstractCash, v=zero(c), args...; atol=ATOL, dothrow=false, kwargs...)
    ...
    elseif isapprox(val, v; atol)
        ...
        if dothrow
            throw(DomainError(val, "cash $val is below the required minimum $(value(v)) + atol $atol"))
        else
...
$(TYPEDSIGNATURES)
"""
mul!(c::Cash, v, args...; kwargs...) = (lock(c.lock) do; _fvalue(c)[] *= v; end; c)
@doc """Inplace remaineder division for Cash objects.

$(TYPEDSIGNATURES)
"""
rdiv!(c::Cash, v, args...; kwargs...) = (lock(c.lock) do; _fvalue(c)[] /= v; end; c)
@doc """Inplace division for Cash objects.

$(TYPEDSIGNATURES)
"""
div!(c::Cash, v, args...; kwargs...) = (lock(c.lock) do; _fvalue(c)[] ÷= v; end; c)
@doc """Inplace modulo for Cash objects.

$(TYPEDSIGNATURES)
"""
mod!(c::Cash, v, args...; kwargs...) = (lock(c.lock) do; _fvalue(c)[] %= v; end; c)
@doc """Sets the cash object to v.

$(TYPEDSIGNATURES)
"""
cash!(c::Cash, v, args...; kwargs...) = (lock(c.lock) do; _fvalue(c)[] = v; end; c)
