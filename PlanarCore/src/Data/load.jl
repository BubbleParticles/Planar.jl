...
function _load_pairdata(zi, exc_name, k, timeframe; kwargs...)
    (pair_df, za) = load(zi, exc_name, k, timeframe; with_z=true, kwargs...)
    return k => PairData(k, timeframe, pair_df, za)
end

function _load_zarr(zi, exc_name, k, timeframe; kwargs...)
    (loaded, _) = load(zi, exc_name, k, timeframe; as_z=true, kwargs...)
    return k => loaded
end
...
function load_ohlcv(
    ...
    sizehint!(out, length(results))
    for (p, r) in zip(pairs, results)
        r.second === nothing && continue
        out[p] = r.second
    end
    out
end
