...
function _fetch_ohlcv_from_to(
    ...
    # Defensive guard: skip fetch when the range is inverted/empty
    if from isa Number && to isa Number
        ...
        # Only skip if from is in the future (beyond a small clock-skew buffer).
        # Do NOT skip if from is just slightly in the past (recent data fetch is valid).
        if from > timefloat(now()) + 60000.0
            @warn "fetch_ohlcv_from_to: skipping fetch — from is in the future" pair timeframe from to maxlog=10
            return empty_ohlcv()
        end
    end
