...
function kill_task(t)
  try
    interrupt_task = errormonitor(@async Base.throwto(t, InterruptException()))
...
"""
start_task(t::Task, state) = (init_task(t, state); errormonitor(t); schedule(t); t)
    ...
    try
        errormonitor(@async begin
            try
                ...
                e isa InterruptException && rethrow(e)
                @error "waitforcond watchdog failed" exception=(e, catch_backtrace())
            finally
                safenotify(cond)
            end
        end)
...
# Interruptible sleep that checks a running flag
function sleep_pad_interruptible(from, throttle, running_flag::Ref{Bool})
    elp = round(throttle - (now() - from), Millisecond)
    if elp <= Second(0)
        return
    end
    # Sleep in small chunks and check the running flag
    chunk = min(throttle, Second(1))
    remaining = elp
    while remaining > Second(0) && running_flag[]
        sleep_time = min(chunk, remaining)
        sleep(sleep_time)
        remaining = round(remaining - sleep_time, Millisecond)
    end
end

export waitforcond, start_task, stop_task, istaskrunning, @istaskrunning, @start_task, sleep_pad, sleep_pad_interruptible
