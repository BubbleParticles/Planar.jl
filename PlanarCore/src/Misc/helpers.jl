...
function queryfromstruct(T::Type, sep=","; kwargs...)
    query = try
        ...
        error isa ArgumentError && @error "Wrong query parameters for ($(T))."
        rethrow(error)
    end
    params = Dict()
...
roundfloat(val, prec) = begin
    # A precision of 0 (or negative) is meaningless: `1.0 / prec` would be
    # Inf, poisoning the result with NaN and silently corrupting downstream
    # cash/position math (e.g. CurrencyCash construction when an exchange
    # reports `precision: 0`). Treat it as "no rounding" and return `val`.
    prec <= 0 && return val
    inv_prec = 1.0 / prec
