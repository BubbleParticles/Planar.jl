...
macro get(dict, k, expr)
    ...
    expr = esc(expr)
    k = esc(k)
    :(@_coalesce get($dict, $k, missing) $expr)
end
...
macro lget!(dict, k, expr)
    ...
    k = esc(k)
    quote
        @_coalesce get($dict, $k, missing) $dict[$k] = $expr
    end
end
...
macro _coalesce(args...)
    # Custom coalesce that treats both `missing` and `nothing` as "not found"
    # This is critical for JSON data where `null` becomes `nothing` in Julia
    if length(args) < 2
        throw(ArgumentError("@_coalesce requires at least 2 arguments"))
    end
    # Build: let v = first; if v === missing || v === nothing; v = second; end; ...; v end
    v = gensym("_coalesce_v")
    body = Expr(:block, :($v = $(esc(args[1]))))
    for arg in args[2:end]
        push!(body.args, :(if $v === missing || $v === nothing; $v = $(esc(arg)); end))
    end
    push!(body.args, v)
    Expr(:let, Expr(:block, Expr(:(=), v, :())), body)
end
...
@generated function fromdict(tuple, key, di, kconvfunc=convert, convfunc=convert)
    ...
    for (fi, ty) in zip(fieldnames(ttype), fieldtypes(ttype))
        # Index with `get(..., nothing)` so a partial API response that omits an
        # optional key (e.g. ccxt trade `fee`/`fees`/`order`) does not throw a
        # bare `KeyError` that would crash the caller's deserialization loop
        # (watcher `_process!`). The missing value is passed to `convfunc` as
        # `nothing`; optional/`Nothing`-union field types accept it, and callers
        # supply a `convfunc` (e.g. `_wconvert`) that maps `nothing` to a safe
        # default (empty collection, zero fee) for collection/required fields.
        # Fix for JSON null -> nothing: if field type is Union{Nothing, T} and
        ...
        if ty isa Union && any(t -> t === Nothing, ty.a isa Union ? ty.a : (ty.a, ty.b))
            ...
            p = Expr(:kw, fi, :(convfunc($ty, $val_expr)))
        end
        push!(params.args, p)
    end
    ex
...
macro writeerror(filehandle)
    quote
        f = $(esc(filehandle))
        # Use the caller's `now()` (UTC via TimeTicks) rather than a module-
        # qualified `Dates.now()` which (a) throws UndefVarError when the calling
        # module hasn't imported `Dates` and (b) reports local wall-clock time.
        println(f, string($(esc(:now))()))
...
function _dedup_funcs(st::Vector{Base.StackFrame})
    ...
    for frame in st
        ...
        # Only closure frames contain '#'; plain function names have none.
        # Without the limit/split-safe handling a BoundsError would crash
        # @caller (and any stacktrace-based logging) for ordinary functions.
        suffix = occursin("#", name) ? split(name, "#")[2] : name
        push!(fnames, (suffix, string(":", frame.line)))
    end
