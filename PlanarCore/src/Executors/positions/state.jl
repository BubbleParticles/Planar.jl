...
_inv(::Long, leverage, mmr) = leverage <= 0 ? 1.0 + mmr : 1.0 - 1.0 / leverage + mmr
_inv(::Short, leverage, mmr) = leverage <= 0 ? 1.0 - mmr : 1.0 + 1.0 / leverage - mmr
...
const LIQUIDATION_BUFFER =
    parse(DFT, get(ENV, "PLANAR_LIQUIDATION_BUFFER", "0.02")) |> abs |> negate
