...
function test_update_ohlcv_1(s)
    ...
    lm.update!(s, ii, :a, :b) do ohlcv, from_date
        # Do nothing
    end
    @test !hasproperty(ohlcv, :a)
    @test !hasproperty(ohlcv, :b)
    append!(ohlcv_dict(ii)[tf], da.to_ohlcv(sml.synthohlcv()); cols=:union)
    first_date = lm.firstdate(ohlcv)
    rows = da.nrow(ohlcv)
    this_update = Ref{Any}()
    @assert !isempty(ohlcv_dict(ii)[tf])
    @info "test" objectid(ohlcv_dict(ii)[tf])
    lm.update!(s, ii, :a, :b) do ohlcv, from_date
    ...
    @test_warn "no new data" lm.update!(s, ii, :a, :b) do ohlcv, from_date
        # ohlcv is empty
    end
    ...
    @test_warn "wrong number" lm.update!(s, ii, :a, :b) do ohlcv, from_date
    ...
    delete!(s.attrs, :lastdate_updates)
    @test_warn "missing entries" lm.update!(s, ii, :a, :b) do ohlcv, from_date
        n = lm.nrow(ohlcv)
        rand(n - 100, 2)
    end

    delete!(s.attrs, :lastdate_updates)
    @test_warn "keeping end" lm.update!(s, ii, :a, :b) do ohlcv, from_date
        n = lm.nrow(ohlcv)
        rand(n + 100, 2)
    end
end
