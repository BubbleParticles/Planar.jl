...
function test_processing()
    @eval begin
        ...
        using Planar.Engine.Data.TimeTicks: TimeFrame, Minute, DateTime
        PlanarDev.Planar.@environment!

        proc = Planar.Engine.Processing
        @testset "Processing.upsample" begin
            ...
            result = proc.upsample(df, tf_large, tf_small)
            ...
            result1 = proc.upsample(df1, tf_large, tf_small)
            ...
            result_nonmono = proc.upsample(df_nonmono, tf_large, TimeFrame(Minute(1)))
            ...
            proc.upsample(df1, tf_large, TimeFrame(Minute(1)))
            df_nan = da.DataFrame(timestamp=[DateTime(2024,1,1,0,5)], open=[NaN], high=[Inf], low=[-Inf], close=[missing], volume=[10.0])
            @test_throws MethodError proc.upsample(df_nan, tf_large, TimeFrame(Minute(1)))
            ...
            result_dup = proc.upsample(df_dup, tf_large, TimeFrame(Minute(1)))
            ...
            result_large = proc.upsample(df_large, tf_large, TimeFrame(Minute(1)))
