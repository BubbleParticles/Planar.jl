...
function test_orderscount(s)
    ...
    t = ect.call!(s, ai, ect.GTCOrder{ect.Buy}; amount, price, date=date())
    @test ismissing(t)
    @test_throws AssertionError ect.call!(s, ai, ect.GTCOrder{ect.Buy}; amount, price, date=date())
    ...
    ect.call!(s, ai, ect.GTCOrder{ect.Buy}; amount, price, date=date(2))
    ...
    Main.s = s
    amount *= 3
    ect.call!(s, ai, ect.MarketOrder{ect.Buy}; amount, date=date(3))
    @test s.cash < maxcost
    ...
    t = ect.call!(s, ai, ect.GTCOrder{ect.Sell}; amount, price=ai.limits.price.min, date=date(3))
    @test !isnothing(t)
    @test t.order isa ect.GTCOrder{ect.Sell}
    @info "TEST: orders" cash(ai) prev_amount amount
    @test cash(ai) == prev_amount
    ect.call!(s, ai, ect.GTCOrder{ect.Sell}; amount, price=11ai.limits.price.min, date=date(3))
