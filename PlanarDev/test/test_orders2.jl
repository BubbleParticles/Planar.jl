...
function test_orders2()
    @testset "orders" begin
        @eval begin
            ...
            using .Planar.Engine.Instances: NoMarginInstance, MarginInstance
            using .Planar.Engine.Strategies: MarginStrategy, Strategy
            using .ect:
                committment,
                withfees,
                amount_with_fees,
        ...
        doreset() = (Planar.Engine.Strategies.reset!(s); Planar.Engine.Strategies.reset!(s_nomg))
        @testset failfast = FAILFAST Base.invokelatest(test_committment, s, s_nomg)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_buysell, s)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_buysell, s_nomg)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_hasorders, s)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_hasorders, s_nomg)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_orderiterator, s)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_orderiterator, s_nomg)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_unfillment, s)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_unfillment, s_nomg)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_iscommittable, s)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_iscommittable, s_nomg)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_orderscount, s)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_orderscount, s_nomg)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_hascash, s)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_hascash, s_nomg)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_buyorders_sellorders, s)
        doreset()
        @testset failfast = FAILFAST Base.invokelatest(test_buyorders_sellorders, s_nomg)
    end
end
